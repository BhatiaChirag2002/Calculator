import 'package:flutter/material.dart';

class AppLightModeColors {
  static Color backgroundColor = const Color(0xffecf0f3);
  static Color buttonBackgroundColor = const Color(0xffe0e1e5);
  static Color buttonColor = const Color(0xff4d12f5);
  static Color buttonNumColor = const Color(0xffffffff);
  static Color buttonHighlightColor = const Color(0xff024d59);
  static Color buttonInsideColor = const Color(0xffffffff);
  static Color textColor = const Color(0xff000000);
}

class AppDarkModeColors {
  static Color backgroundColor = const Color(0xff1c1c1c);
  static Color buttonBackgroundColor = const Color(0xff282828);
  static Color buttonColor = const Color(0xfff57b00);
  static Color buttonNumColor = const Color(0xFF000000);
  static Color buttonHighlightColor = const Color(0xff5ed5e1);
  static Color buttonInsideColor = const Color(0xffffffff);
  static Color textColor = const Color(0xFFFFFFFF);
}
